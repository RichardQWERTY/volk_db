...
def show_full_exhibit_card():
    ...
    try:
        ...
        #вызов функции из PostgreSQL
        cur.execute("""
            SELECT * FROM get_full_exhibit_card(%s)
        """, (inv_num,))
...
def show_exhibits_in_exhibitions():
    ...
    try:
        #вызов функции из PostgreSQL
        cur.execute("""
            SELECT * FROM get_exhibits_in_exhibitions()
        """)
        rows = cur.fetchall()
        if not rows:
...
def main():
    while True:
        ...
        if choice == '1':
            ...
            print("нужно выбрать от 0 до 11")
